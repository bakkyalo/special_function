...
\begin{tikzpicture}[domain = -1.5:4, samples = 100, thick]	% sampleじゃなくてsamples
  \draw (0, 0) node[below left]{\Large O};		% 原点
  ...
  % 軸の描画
  \draw [thick, -stealth] (0, 0)--(8, 0) node[right] {\Large$m$};		% x軸
  \draw [thick, -stealth] (0, 0)--(0, 6) node[above] {\Large$i$};		% y軸
  ...
  \foreach \i in {0, 1, 2, 3, 4, 5} 
    \draw[domain = {\i + 0.5}:{-0.5}, -stealth] plot (\x, {-\x+\i}) 
	node [left, fill=white] {\Large$n=\i$}; 
